...
class Simulator {
            ...
            while (neighborIdx != -1) {
              ...
              if (rSq < kernelRadiusSq) {
                ...
                if (addSprings && selfIdx < neighborIdx && r > minDist) {
                  ...
                  const springHashIdx = selfIdx * maxParticles + neighborIdx;
                  if (!this.springPresent[springHashIdx]) {
                    this.springPresent[springHashIdx] = r;
  ...
  applySpringDisplacements(dt) {
    ...
    for (let spring of Object.keys(this.springPresent)) {
      const springIdx = parseInt(spring);
      const i = Math.floor(springIdx / maxParticles);
      const j = springIdx % maxParticles;

      const p0 = this.particles[i];
      const p1 = this.particles[j];

      let restLength = this.springPresent[spring];
      ...
      if (dist > restLength + tolerableDeformation) {
        ...
        this.springPresent[spring] = restLength;
      ...
      if (restLength > kernelRadius) {
        delete this.springPresent[spring];
        continue;
      }
