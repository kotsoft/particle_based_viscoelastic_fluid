...
class Simulator {
          ...
          for (let bucketDY = -1; bucketDY <= 1; bucketDY++) {
            ...
            while (neighborIdx != -1) {
              ...
              if (rSq < kernelRadiusSq) {
                ...
                if (addSprings && selfIdx < neighborIdx && r > minDist) {
                  const key = selfIdx * 10000 + neighborIdx;
                  if (!this.springExists[key]) {
                    this.springExists[key] = true;
                    p0.springs[neighborIdx] = r;

                    this.springs.push(new Spring(selfIdx, neighborIdx, r));
                  }
                }
              }
  ...
  applySpringDisplacements(dt) {
    ...
    for (let springIdx = 0; springIdx < this.springs.length; springIdx++) {
      let spring = this.springs[springIdx];
      const restLength = spring.restLength;

      let p0 = this.particles[spring.particleIdxA];
      let p1 = this.particles[spring.particleIdxB];

      let dx = p1.posX - p0.posX;
      let dy = p1.posY - p0.posY;
      let dist = Math.sqrt(dx * dx + dy * dy);

      const tolerableDeformation = yieldRatio * restLength;

      if (dist > restLength + tolerableDeformation) {
        spring.restLength = restLength + plasticity * (dist - restLength - tolerableDeformation);
      } else if (dist < restLength - tolerableDeformation && dist > minDist) {
        spring.restLength = restLength - plasticity * (restLength - tolerableDeformation - dist);
      }

      if (spring.restLength < minDist) {
        spring.restLength = minDist;
      }

      if (spring.restLength > kernelRadius) {
        deleteSprings.push(springIdx);
        continue;
      }

      let D = springStiffness * (1 - spring.restLength * kernelRadiusInv) * (dist - spring.restLength) / dist;
      dx *= D;
      dy *= D;
    ...
    // for (let i = deleteSprings.length - 1; i >= 0; i--) {
    //   this.springs.splice(deleteSprings[i], 1);
    // }
    ...
    //       const key = particle.idx * 10000 + springIdx;
    //       delete this.springExists[key];
